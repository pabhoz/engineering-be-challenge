          ...
          "event": [
            {
              "listen": "test",
              "script": {
                "exec": ["pm.test(\"Setting JWT Token\", function () {", "    var jsonData = pm.response.json();", "    console.log(jsonData);", "    pm.collectionVariables.set(\"client_token\", jsonData.access_token);", "});"],
